...
if __name__ == "__main__":
    ...
    train_predictions = torch.load(os.path.join(args.load, "models", MODEL_NAME, "train_predictions.pt")).cpu().numpy()
    train_y = torch.load(os.path.join(args.load, "models", MODEL_NAME, "train_y.pt")).cpu().numpy()

    val_predictions = torch.load(os.path.join(args.load, "models", MODEL_NAME, "val_predictions.pt")).cpu().numpy()
    val_y = torch.load(os.path.join(args.load, "models", MODEL_NAME, "val_y.pt")).cpu().numpy()
    ...
    # Add model data
    for i in range(np.shape(train_predictions)[1]):
        train[f"model::train_predictions_{i}"] = train_predictions[:, i]
    train["model::train_y"] = train_y

    for i in range(np.shape(val_predictions)[1]):
        val[f"model::val_predictions_{i}"] = val_predictions[:, i]
    val["model::val_y"] = val_y
    
    for i in range(np.shape(test_predictions)[1]):
        test[f"model::test_predictions_{i}"] = test_predictions[:, i]
    test["model::test_y"] = test_y

    # Subset train and val for max prediction but *do not subset test* (this is done per-OOD detector)
    train = train.loc[train.groupby("healthCode")["model::train_predictions_1"].idxmax()]
    val = val.loc[val.groupby("healthCode")["model::val_predictions_1"].idxmax()]
    ...
    for key, ood_detectors in ood_detectors_performance.items(): # key in ("success", "fail")
        ...
        for countplot_dodge in (True, False):
            ...
            for i, ood_detector in tqdm.tqdm(enumerate(ood_detectors), desc=f"Plot {key}", total=len(ood_detectors)):
                ...
                test_plot = test.loc[test.groupby(["healthCode", f"ood_detector::{ood_detector}::prediction"])["model::test_predictions_1"].idxmax()]
                test_plot.to_csv(os.path.join(SAVE_DATA_PATH, f"test_{ood_detector}.csv"))
                    ...
                    if demographic_var_type == "categorical":
                        if countplot_dodge:
                            sns.countplot(
                                ...
                                data = test_plot, ax = ax, dodge = countplot_dodge, palette = palette, hue_order=["ID", "OOD"],
                            )
                            ax.set_xticklabels([truncate(s.get_text()) for s in ax.get_xticklabels()], rotation=45, ha="right")
                            ax.legend(title="Prediction")
                            ax.set(xlabel=None, ylabel=None)
                        else:
                            test_plot[demographic_var] = test_plot[demographic_var].astype(str)
                            sns.countplot(x=demographic_var, hue = f"ood_detector::{ood_detector}::prediction",
                                          data=test_plot, ax=ax, dodge=False, palette=palette, hue_order=["ID", "OOD"])

                            counts = pd.crosstab(test_plot[demographic_var], test_plot[f"ood_detector::{ood_detector}::prediction"])
                        ...
                        sns.histplot(
                            ...
                            data = test_plot, ax = ax, legend=True,
                        )
                        ax.legend(
    ...
    if DATASET == "isic":
        ...
        masks = {
            "asexual_stage_other": ~np.array(test["Asexual.stage..hpi."].isin([12, 14, 16]), dtype=bool),
        }
        masks["asexual_stage_12_14_16"] = ~masks["asexual_stage_other"]
        ...
        masks = {
            "none": np.array(test["deep-brain-stimulation"] != 1.0, dtype=bool),
        }
        masks["deep_brain_stimulation"] = ~masks["none"]
        
        indices = {
            group: np.arange(len(test), dtype=int)[masks[group]] for group in masks.keys()
        }
        for replicate in tqdm.tqdm(list(range(1000)) + ["main",], desc="Bootstrap PDDB", total=1001):
        ...
        ax.set_title("Accuracy and AUROC for retired in test set")
        ax.legend(title="metric")
