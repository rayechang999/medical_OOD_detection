# Training model on ISIC MSKCC subset
...
class InstanceClassificationModel(nn.Module):
    def __init__(self, model, num_classes):
        ...
        self.model = model
        self.classifier = nn.Linear(32, out_features=num_classes)
...
def train_instance_classification(
    ...
    model = InstanceClassificationModel(model, len(dataloader.dataset))
...
if __name__ == "__main__":
    ...
    CONFIG = {
        ...
        "resume": args.resume,
        "dataset": {
            "data_path": "/path/to/Challenge_2020_Training/download",
            "metadata_path": "/path/to/Challenge_2020_Training/challenge-2020-training_metadata_2024-11-22.csv"
        },
        ...
        "model": {
            ...
            "params": {
                ...
                "efficientnet": "B3",
                "fine_tune": False,
                "batch_size": 32, # 32 may lead to better performance
                "shuffle": True,
                "epochs": 15, # originally 15
                "train_frac": 0.80,
            }
        },
        "ood_detectors": {
    ...
    # Define subsets of the data: GLOBAL selects which sound we use
    rng = np.random.default_rng(seed=0)

    print(np.unique(dataset.obs["benign_malignant"], return_counts=True))
    # there are only 584 malignant samples
    subsample_negative = rng.choice(np.argwhere(dataset.obs["benign_malignant"] == "benign").squeeze(), size=584, replace=False)
    subsample_positive = rng.choice(np.argwhere(dataset.obs["benign_malignant"] != "benign").squeeze(), size=584, replace=False)
    GLOBAL_SUBSET = np.array([False for _ in range(len(dataset))])
    GLOBAL_SUBSET[subsample_negative] = True
    GLOBAL_SUBSET[subsample_positive] = True
    
    # Filter out nan values
    GLOBAL_SUBSET = (
        GLOBAL_SUBSET
        & (~pd.isna(dataset.obs["age_approx"]))
        & (~pd.isna(dataset.obs["sex"]))
        & (~pd.isna(dataset.obs["anatom_site_general"]))
    )
    print(type(GLOBAL_SUBSET)) #
    ...
    ALL_TRAIN_SUBSET = GLOBAL_SUBSET & np.array(dataset.obs["attribution"] == "Memorial Sloan Kettering Cancer Center")
    TEST_SUBSET = GLOBAL_SUBSET & np.array(dataset.obs["attribution"] != "Memorial Sloan Kettering Cancer Center")

    print(f"Size of train indices: {np.sum(ALL_TRAIN_SUBSET)} / {np.sum(TEST_SUBSET)}")

    # Move 20% of ALL_TRAIN_SUBSET to TEST_SUBSET
    all_train_indices = np.where(ALL_TRAIN_SUBSET == 1)[0]
    rng.shuffle(all_train_indices)
    split_point = int(len(all_train_indices) * 0.2)
    TEST_SUBSET[all_train_indices[:split_point]] = 1
    ALL_TRAIN_SUBSET[all_train_indices[:split_point]] = 0

    print(f"Size of train indices: {np.sum(ALL_TRAIN_SUBSET)} / {np.sum(TEST_SUBSET)}")

    # can also do age, attribution, sex
    all_train_indices = np.where(ALL_TRAIN_SUBSET == 1)[0]
    rng.shuffle(all_train_indices)
    split_point = int(len(all_train_indices) * CONFIG["model"]["params"]["train_frac"])
    TRAIN_SUBSET = np.zeros_like(ALL_TRAIN_SUBSET)
    VAL_SUBSET = np.zeros_like(ALL_TRAIN_SUBSET)

    TRAIN_SUBSET[all_train_indices[:split_point]] = 1
    VAL_SUBSET[all_train_indices[split_point:]] = 1
    
    # Subset and prepare for training
    train_dataset = dataset.subset(TRAIN_SUBSET, inplace=False)
    val_dataset = dataset.subset(VAL_SUBSET, inplace=False)
    ...
    IMAGE_SIZE = None
    if "EfficientNet" not in CONFIG["model"]["name"]:
        IMAGE_SIZE = 128
    elif "EfficientNet" in CONFIG["model"]["name"] and CONFIG["model"]["params"]["efficientnet"] == "B3":
        IMAGE_SIZE = 300
    else:
        raise Exception
    ...
    base_transforms = torchvision.transforms.Compose([
        datasets.transforms.isic.ApplyTo(torchvision.transforms.Resize((IMAGE_SIZE, IMAGE_SIZE)), index=0),
        # datasets.transforms.isic.ApplyTo(torchvision.transforms.RandomHorizontalFlip(), index=0),
        # datasets.transforms.isic.ApplyTo(torchvision.transforms.RandomRotation(180), index=0),
        datasets.transforms.isic.ApplyTo(torchvision.transforms.ToTensor(), index=0),
    ])

    # ---- Load in the model ----
    # sex, age_approx, anatom_site_general
    map_sex = {sex: i for i, sex in enumerate(dataset.data.obs["sex"].unique())} # use full dataset here to account for all possibilities
    map_anatom_site_general = {site: i for i, site in enumerate(dataset.data.obs["anatom_site_general"].unique())}
    ...
    model_transforms = torchvision.transforms.Compose([
        ...
        datasets.transforms.isic.ApplyTo(torchvision.transforms.RandomRotation(180), index=0),
        datasets.transforms.isic.ApplyTo(torchvision.transforms.ToTensor(), index=0),
        datasets.transforms.isic.AddMetadata(
            metadata_funcs = {
                "sex": lambda x: nn.functional.one_hot(torch.tensor(map_sex[x], dtype=torch.long), num_classes=len(map_sex)).view(-1,),
                "age_approx": lambda x: torch.tensor(float(x), dtype=torch.get_default_dtype()).view(-1,),
                "anatom_site_general": lambda x: nn.functional.one_hot(torch.tensor(map_anatom_site_general[x], dtype=torch.long), num_classes=len(map_anatom_site_general)).view(-1,)
            }
        ),
    ...
    nets = {key: {"model": models.EfficientNet(
        model = f"{CONFIG['model']['params']['model']}",
        efficientnet = CONFIG["model"]["params"]["efficientnet"],
        fine_tune = CONFIG["model"]["params"]["fine_tune"],
        metadata_in_features = len(map_sex) + 1 + len(map_anatom_site_general),
    ...
    if RESUME is not None and 2 < RESUME:
        ...
        for key in tqdm.tqdm(nets.keys(), "Compute model embeddings"):
            ...
            with torch.no_grad():
                embedding = torch.cat([nets[key]["model"].model(x, return_latent=True) for x, y in model_train_dataloader_fixed], dim=0)
                if "logit_norm" in key:
                    embedding = F.normalize(embedding, p=2, dim=1) / CONFIG["ood_detectors"]["logit_norm_temperature"]
                y = torch.cat([y for x, y in model_train_dataloader_fixed], dim=0)
            ...
            nets[key]["train_embedding_dataloader"] = torch.utils.data.DataLoader(
                MatrixDataset(nets[key]["train_embedding"], y),
                batch_size = 32,
                shuffle = False,
            )

            with torch.no_grad():
                embedding = torch.cat([nets[key]["model"].model(x, return_latent=True) for x, y in model_test_dataloader], dim=0)
                if "logit_norm" in key:
                    embedding = F.normalize(embedding, p=2, dim=1) / CONFIG["ood_detectors"]["logit_norm_temperature"]
    ...
    model_intermediate_save_path = os.path.join(args.save, "models", "base")
