# This is just benchmark_pddb.py ported over to malaria
...
def train_instance_classification(
    ...
    class IndexedDataset:
        ...
        def __getitem__(self, key):
            x, _ = self.dataset[key] # _ is y, not metadata
...
if __name__ == "__main__":
    ...
    train_indices = dataset.obs.index[dataset.obs["Country"] != "Myanmar"].tolist()
    train_indices = np.array(train_indices)

    shuffled_train_indices = rng.permutation(train_indices)
    split_idx = int(0.8 * len(shuffled_train_indices))

    train_inds = shuffled_train_indices[:split_idx] # only keep 80%; the rest 20% are in the test set

    ALL_TRAIN_SUBSET = np.zeros(len(dataset), dtype=bool)
    ALL_TRAIN_SUBSET[train_inds] = True
    TEST_SUBSET = ~ALL_TRAIN_SUBSET

    temp_dataset = dataset.subset(ALL_TRAIN_SUBSET, inplace=False)

    shuffled_train_inds = rng.permutation(np.arange(len(temp_dataset)))
    val_split_idx = int(CONFIG["model"]["params"]["train_frac"] * len(temp_dataset))
    final_train_inds = shuffled_train_inds[:val_split_idx]

    TRAIN_SUBSET = np.zeros(len(temp_dataset), dtype=bool)
    TRAIN_SUBSET[final_train_inds] = True
    ...
    METADATA_TO_Y_FUNC = lambda metadata: int(metadata["ClearanceRate_binary"])

    # Collect the full dataset as a numpy matrix so that we can train the rank normalization
    x_rank_norm = np.concatenate([np.expand_dims(train_dataset[i][0], axis=0) for i in range(len(train_dataset))], axis=0)
    rank_norm_transform = datasets.transforms.malaria.RankNormalization(data=x_rank_norm)
    
    base_transforms = torchvision.transforms.Compose([
        datasets.transforms.malaria.ApplyTo(
            rank_norm_transform,
            index = 0,
        ),
    ])
    ...
    model_transforms = torchvision.transforms.Compose([
        base_transforms,
        datasets.transforms.malaria.ApplyTo(
            datasets.transforms.malaria.RandomMagnitudeRescaling(scale_factor_min=0.8, scale_factor_max=1.2),
            index = 0
        ),
        datasets.transforms.malaria.ApplyTo(
        ...
        datasets.transforms.malaria.ApplyTo(
            datasets.transforms.malaria.ConvertYToTensor(
    ...
    # ---- Bootstrap results ----
    model_intermediate_save_path = os.path.join(args.save, "models", "lightgbm")
    os.makedirs(model_intermediate_save_path)
    ...
    model_train_metrics = model.train(
        dataloader = model_train_dataloader_deterministic,
        num_round = 1000,
        val_dataloader = model_val_dataloader,
        save = model_intermediate_save_path,
    )
    with open(os.path.join(model_intermediate_save_path, "train_metrics.json"), "w") as f:
        f.write(json.dumps(model_train_metrics, indent=2))
    
    all_out, all_y = model.predict(model_test_dataloader, return_y=True, device=DEVICE)

    if not torch.all(torch.abs(torch.sum(all_out, dim=1) - 1.0) < 1e-4):
        print(torch.all(torch.abs(torch.sum(all_out, dim=1) - 1.0) < 1e-4))
        print(all_out)
        raise Exception
